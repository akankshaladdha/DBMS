...
insert into movcast values(301,1002,'heroine'),
(301,1001,'heroine'),
(303,1003,'hero'),
(303,1002,'guest'),
(304,1004,'hero');
...
select title from movies m, movcast mc
where m.mov_id = mc.mov_id and act_id in (
											select act_id from movcast group by act_id having count(act_id)>1)
                                            group by title having count(*) >1;
                                            
select act_name, title ,year 
from actor a 
JOIN movcast c on a.act_id = c.act_id 
